...
LLM 对话协议重构分 5 个阶段（P1-P5），当前 P1-P4 全部完成。
...
| **P4** 前端 runtime | 6 atom 拆分，tool_status 工具卡片，openThread 融合 | 否 | ✅ 全部完成（6/6 Tasks） |
...
### F. P4 前端 runtime 6 Atom 重构（100% 完成）

- ✅ 6 Atom 拆分：messages / connection / error / threadMeta / runState / interruptState
- ✅ 独立 Emitter：每个 atom 有自己的事件发射器，精确订阅
- ✅ `getSnapshot()` 向后兼容：由 6 atom 实时拼接
- ✅ `use-langgraph-stream` hook selector 接入
- ✅ `toolStatus` 字段 + `ToolCallCard` 组件
- ✅ 取消按钮调用 `stop()` 触发后端 cancel
- ✅ openThread 三段式架构（getState → list → joinActiveStream）
- ✅ 全量测试（20 tests passed）
...
## 待跟进：P5 user 隔离 + metrics

> P1-P4 全部完成，仅剩 P5 治理相关工作。

| Task | 内容 | 关键文件 | 状态 |
|------|------|---------|------|
| **1** | user 隔离 — 控制器层（AuthGuard + CurrentUser） | `threads.controller.ts`, `runs.controller.ts` | ⬜ 待做 |
| **2** | user 隔离 — service 层串接（userId 透传） | `ai.service.ts` | ⬜ 待做 |
| **3** | metrics 埋点（ttfb / token_rate / run_duration / reconnect_count） | 新增 decorator | ⬜ 待做 |
- ✅ stop 统一语义（P2-4）
- ✅ 工具卡片设计稿（P4 设计前置，已完成）
