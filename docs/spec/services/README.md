...
### 4. [文件资源管理器](./file-resource-manager.md)

File Resource Manager - 全局文件资源管理服务

- 单例模式设计
- 资源注册表机制
- 生命周期管理（注册/注销/释放）
- 独立 DisposableStore 设计
- 数据与生命周期分离架构
...
│                                                                  │
│  ┌────────────────┐  ┌────────────────────┐  ┌────────────────┐ │
│  │KeybindingService│ │FileResourceManager │ │  其他服务...     │ │
│  │ - 快捷键绑定    │ │ - 资源注册表        │ │               │ │
│  │ - 快捷键触发    │ │ - 生命周期管理      │ │               │ │
│  └────────────────┘  └────────────────────┘  └────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
...
│   └── file-system/                 # 文件系统服务
│       ├── manager/
│       │   ├── file-resource-manager.ts  # 文件资源管理器
│       │   └── file-handle-cache.ts      # 文件句柄缓存
│       ├── types.ts                 # 类型定义
│       └── index.ts                 # 导出入口
│
└── types/
